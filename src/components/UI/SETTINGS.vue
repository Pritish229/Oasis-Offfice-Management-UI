<template>
  ...
  <a class="btn btn-fixed-end btn-primary btn-icon btn-setting" @click="openOffcanvas" role="button"
  ...
  <div v-show="offcanvasVisible" class="offcanvas-backdrop fade" :class="{ show: showOffcanvas }"
    @click="closeOffcanvas"></div>
  <div v-show="offcanvasVisible" class="offcanvas offcanvas-end" :class="{ show: showOffcanvas }" :style="{
    visibility: offcanvasVisible ? 'visible' : 'hidden',
    transform: showOffcanvas ? 'translateX(0)' : 'translateX(100%)'
  }" id="offcanvasExample" tabindex="-1" aria-labelledby="offcanvasExampleLabel">
...
const closeOffcanvas = () => {
  ...
  }, 300) // match Bootstrap’s transition
}
</script>
