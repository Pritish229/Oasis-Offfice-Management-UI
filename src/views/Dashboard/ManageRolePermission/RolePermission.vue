<template>
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade show active" id="nav-role">
                    ...
                    <section v-if="!permissionview">
                        ...
                        <!-- Role Cards -->
                        <div class="row">
                            <div class="col-lg-4 mb-4" v-for="role in allroles" :key="role._id">
                                <div class="border border-2 rounded-2 p-3">
                                    ...
                                    <small>Created at : {{ formatDate(role.createdAt) }}</small>
                                </div>
                            </div>
                        </div>
                    </section>
                            ...
                            <div class="row">
                                <div class="col-lg-4 col-md-6 mb-3" v-for="perm in allPermissions" :key="perm._id">
                                    <div
                                        ...
                                        <div>
                                            <h5 class="text-muted  mb-1">{{ perm.name }}</h5>
    ...
    <BaseModal ref="editModal" modalId="editModal" title="Edit Role" :showClose="true" submitButtonText="Update"
        ...
        <Form ref="editFormRef" :validation-schema="schema" @submit="handleUpdate">
            <BaseInput name="name" label="Role Name" :is_required="true" placeholder="Enter Role Name"
                helper="Minimum 5 characters." v-model="editRole.name" />
            <BaseTextArea name="description" label="Description" placeholder="Enter Description"
...
const togglePermission = async (permissionId, allow) => {
    try {
        await axios.post(`${API_URL}/roles/${selectedRole.value._id}/permissions`, {
            permissionId, allow
        });
        if (allow) {
            assignedPermissions.value.push(permissionId);
        } else {
            assignedPermissions.value = assignedPermissions.value.filter(p => p !== permissionId);
        }
    } catch (err) {
...
const allroles = ref([])
const editRole = ref({ id: '', name: '', description: '' })
...
const formatDate = (dob) => dob ? dayjs(dob).format('DD MMM YYYY') : '-'

const openModal = () => customModal.value?.show()
const openEditModal = (role) => {
    editRole.value = { id: role._id, name: role.name, description: role.description }
    editModal.value?.show()
}

const handleCancel = () => formRef.value?.resetForm()
const handleCancelEdit = () => editRole.value = { id: '', name: '', description: '' }
...
const fatchRoles = async () => {
    ...
    if (res.status === 200) allroles.value = res.data
}
...
const handleUpdate = async () => {
    try {
        const res = await axios.put(`${API_URL}/roles/${editRole.value.id}`, {
...
const handleSelect = (option) => {
    const selectedRole = allroles.value.find(r => r._id === option.id)
    if (option.value === 'edit' && selectedRole) {
    ...
    else if (option.value === 'permission' && (option.id != null || option.id != undefined)) {
        permissionview.value = true
        fetchPermissions()
        fetchRolePermissions(option.id)
        currentRole.value = selectedRole.name
    }
}
